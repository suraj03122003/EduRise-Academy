...
export const purchaseCourse = async (req, res) => {
    try {
        ...
        const alreadyPurchased = await CoursePurchase.findOne({ user: userId, course: courseId });
        if (alreadyPurchased) {
            return res.status(400).json({ success: false, message: "You have already purchased this course." });
        }

        // Save the course purchase record
        await CoursePurchase.create({ user: userId, course: courseId });
